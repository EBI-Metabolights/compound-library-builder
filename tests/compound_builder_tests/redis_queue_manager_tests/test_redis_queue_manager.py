...
from tests.compound_builder_tests.redis_queue_manager_tests.fixtures import (
    compound_redis_queue_manager_fixture,
    compound_list_fixture,
)
...
class TestCompoundRedisQueueManager:
    ...
    def test_populate_queue_sad(self, mock_print, compound_redis_queue_manager_fixture):
        ...
        mock_print.assert_called_once_with(
            "Queue populated. Risk of duplication. Aborting."
        )

    def test_get_compound_ids(
        self, compound_list_fixture, compound_redis_queue_manager_fixture
    ):
        """
        ...
        crqm_fixt.session.get.assert_called_once_with(
            "http://www.ebi.ac.uk/metabolights/ws/compounds/list"
        )
        ListUtils.get_delta.assert_called_once_with(
            compound_list_fixture, DirUtils.get_mtblc_ids_from_directory("none")
        )

    @patch("builtins.print")
    def test_push_compound_ids_to_redis_happy(
        self, mock_print, compound_redis_queue_manager_fixture, compound_list_fixture
    ):
        """
        ...
        crqm_fixt.redis_client.push_to_queue.assert_called_once_with(
            "compounds", dumped
        )

    @patch("builtins.print")
    def test_push_compound_ids_to_redis_sad(
        self, mock_print, compound_redis_queue_manager_fixture, compound_list_fixture
    ):
        """
        It should push a stringified version of a sublist to the compounds queue
    ...
    def test_consume_queue(
        self, compound_redis_queue_manager_fixture, compound_list_fixture
    ):
        """
        ...
        crqm_fixt = compound_redis_queue_manager_fixture
        crqm_fixt.redis_client = MagicMock()
        crqm_fixt.redis_client.consume_queue = MagicMock(
            return_value=str(compound_list_fixture)
        )
