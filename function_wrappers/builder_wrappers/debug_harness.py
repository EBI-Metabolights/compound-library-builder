    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if not enabled:
                ...
                stats.increment('compounds_with_ms') if len(result['spectra']['MS']) > 0 else None
                stats.increment('total_ms_files', len(result['spectra']['MS']))
                stats.increment('compounds_with_nmr') if len(result['spectra']['NMR']) > 0 else None
                stats.increment('total_nmr_files', len(result['spectra']['NMR']))
                stats.increment('wiki') if len(result['pathways']['WikiPathways']) > 0 else None
                stats.increment('reactomepathways') if len(result['pathways']['ReactomePathways']) > 0 else None
